...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⏪ 𝐁𝐚𝐜𝐤", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"📃 𝙿𝚊𝚐𝚎𝚜 {round(total/10)}", callback_data="pages")]
        )
        btn.append(
        ...
        btn.append([InlineKeyboardButton(f"🔰 {round(total/10)}𝐏𝐀𝐆𝐄𝐒", callback_data="pages"), InlineKeyboardButton(" 𝐍𝐞𝐱𝐭 ⏩", callback_data=f"next_{req}_{key}_{n_offset}")])
        ...
        btn.append(
            [
                ...
                InlineKeyboardButton(f"🔰 {round(total/10)} 𝐏𝐀𝐆𝐄𝐒", callback_data="pages"),
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("കണ്ണാപ്പിയെ 🤸‍♀️ Don't Be Oversmart 😒",show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"🔰 {round(int(total_results)/10)}𝐏𝐀𝐆𝐄𝐒",callback_data="pages"), InlineKeyboardButton(text="𝐍𝐞𝐱𝐭 ⏩",callback_data=f"next_{req}_{key}_{offset}")]
        )
        btn.append(
        ...
        btn.append(
            [InlineKeyboardButton(text="🔰 1 𝐏𝐀𝐆𝐄",callback_data="pages")]
        )
        btn.append(
